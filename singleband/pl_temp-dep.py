...
def cond_3d():
    ...
    plt.xlabel('$\omega$')
    plt.ylabel('$\delta t_{pp}$')
    # ax.set_zlim((0,100))

    fig = plt.figure('cc3dim')
    plt.clf()
    ax = fig.gca(projection='3d')
    X, Y = np.meshgrid(w, delays)
...
def cond_w_pcolor():
    ...
    plt.pcolormesh(w, w_delay[sl3],nm3(np.abs(ccw[sl3])), cmap=cm, shading='gouraud')
    plt.xlabel('$\omega$')
...
plt.figure('cond-temp-real', figsize=(3,2.8))
plt.clf()
plt.figure('cond-temp-imag', figsize=(3,2.8))
plt.clf()
plt.figure('cond-temp-legend', figsize=(5,5))
plt.clf()
...
for temp in temps[[0,3,5,6,7]]:
    ...
    plt.figure('cond-temp-real')
    plt.plot(wg*u_e*meV_to_THz,np.abs(c.real), label=f'$T={np.round(temp*u_temp,1)}$ K')
    plt.xlabel('Frequency (THz)')
    ...
    plt.loglog(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$T={np.round(temp*u_temp,1)}$ K')
    plt.xlabel('Frequency (THz)')
    plt.xlim((0,6*gap*u_e*meV_to_THz))
    plt.ylim((0,800e4))
    # plt.ticklabel_format(axis="y", style="sci", scilimits=(2,4))
    plt.ylabel('$\sigma\,\'\'$ ($\Omega^{-1}$cm$^{-1}$)')
    # plt.legend()
    ...
    plt.plot(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$T={np.round(temp*u_temp,0)}$ K')
    plt.ylim((0,0.1))
    plt.legend()
...
plt.tight_layout()
savefig('cond-temp-real.pdf')

plt.figure('cond-temp-imag')
# plt.axvline(2*gap*u_e*meV_to_THz, c='k', lw=0.3)
plt.tight_layout()
savefig('cond-temp-imag.pdf', plot=True)
