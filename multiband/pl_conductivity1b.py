    ...
    for j, v in z(vs):
        for k, case in z(fourcases):
            ...
            for d in delays:
                ...
                r2 = sel(A0_pr=0, A0=1, g=case, w=pulse_w, tau=pulse_tau, v=v)
            ...
            if len(cc1) != 0:
                ...
                plt.tight_layout()
                plt.savefig(f'figs3_0/HL-A{i}-v{j}-imp{k}.pdf')
                cond_3d()
                plt.figure('cc3d')
                plt.savefig(f'figs3_0/cond3d_real-A{i}-v{j}-imp{k}.pdf')
                plt.figure('cc3dim')
                plt.savefig(f'figs3_0/cond3d_imag-A{i}-v{j}-imp{k}.pdf')
                cond_pcolor(lw_imp(case))
                plt.savefig(f'figs3_0/cond_pcolor-A{i}-v{j}-imp{k}.pdf')
                # plt.title(f'$\gamma/2\Delta={gammas[s_imp][0]/2/gap}$')
                # plt.tight_layout()
                cond_w_pcolor()
                plt.savefig(f'figs3_0/cond_w_pcolor-A{i}-v{j}-imp{k}.pdf')
                plt.pause(0.01)
