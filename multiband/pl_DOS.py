...
cd = [cleanclean, cleandirty, dirtyclean, dirtydirty]

for i in np.arange(len(cd)):
    for j in np.arange(len(vs)):
        v = vs[j]
        D_inf = []
        plt.figure()
        for w in wss:
            r = sel(first=True,w=w,v=v, g=cd[i])
            t = r['t']
            d = r['d_2'].real
            ch = np.logical_and(t>100, t<180)
            d_inf = np.mean(d[ch,:], axis=0)
            D_inf.append(d_inf)
            plt.plot(t,d)
        plt.xlabel('$t$')
        ...
        plt.title(f'$v={v}$')
        plt.tight_layout()
        plt.savefig(f'delta-infty-imp{i}-v{j}.pdf')

        D_inf = np.array(D_inf)
        plt.figure()
        plt.plot(wss, -D_inf)
        # plt.plot(wss, -np.sum(D_inf,axis=1))
        plt.axvspan(2*d_eq[0], max(wss), fc='gray', alpha=0.2)
        plt.axvspan(2*d_eq[1], max(wss), fc='gray', alpha=0.2)

        # plt.figure()
        # plt.plot(wss, D_inf[:,:]**2)
        # plt.axvspan(2*d_eq[0], max(wss), fc='gray', alpha=0.2)
        # plt.axvspan(2*d_eq[1], max(wss), fc='gray', alpha=0.2)
        ...
        plt.ylabel('$\delta\Delta_{\infty}$')
        plt.xlabel('$\omega$')
        plt.title(f'$v={v}$')
        plt.tight_layout()
        plt.savefig(f'DOS-imp{i}-v{j}.pdf')
