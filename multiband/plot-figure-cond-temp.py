...
plt.clf()
for temp in temps[::3][:-2]:
    r = sel(first=True, t_delay=0, A0_pr=0, T=temp)
    ...
    ls='-'
    plt.plot(wg*u_e*meV_to_THz,np.abs(c.real),ls=ls)
    plt.xlabel('Frequency (THz)')
    plt.xlim((0,5.5))
    plt.ylim((0,0.7e5))
    ...
    plt.plot(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$T={str(int(np.round(temp*u_temp)))}$ K', ls=ls)
    plt.ylim((0,0.1))
    plt.legend()
