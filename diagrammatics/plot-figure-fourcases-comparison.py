...
folder = '../multiband/conductivity-first-order'
...
plt.clf()
gammas1 = gammas[np.array([3])]
gammas2 = gammas[np.array([3,4,5,8,14, 20, 22,24])]
...
for g1 in gammas1:
    ...
    for g2 in gammas2:
        ...
        if g2 >= gammas[14]:
            ...
            ls='-'
        plt.plot(wg*u_e*meV_to_THz,np.abs(c.real), label=f'$\gamma/2\Delta={str(np.round(g[0]/2/gap,1))}$',ls=ls, c=tableau10[i])
        plt.xlabel('Frequency (THz)')
        plt.xlim((0,5.5))
        plt.ylim((0,1.7e5))
        plt.ylim((0,1.1e4))
        ...
        if g1 == gammas1[0]:
            ...
            plt.plot(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$\gamma={str(np.round(g2*10,1))}$ meV',ls=ls, c=tableau10[i])
            plt.ylim((0,0.1))
            plt.legend()

plt.figure('cond-imp-real')
plt.axvline(2*d_eq[0]*u_e*meV_to_THz, c='k', lw=0.3)
plt.axvline(2*d_eq[1]*u_e*meV_to_THz, c='k', lw=0.3)
plt.tight_layout()
savefig('cond-imp-real1.pdf')

plt.figure('cond-imp-imag')
plt.axvline(2*gap*u_e*meV_to_THz, c='k', lw=0.3)
plt.tight_layout()
savefig('cond-imp-imag1.pdf')

plt.figure('cond-imp-legend')
savefig('cond-imp-legend1.pdf')
...
plt.figure('cond-imp-legend', figsize=(5,5))
plt.clf()
gammas1 = gammas[np.array([3,4,5,8,14, 20, 22,24])]
gammas2 = gammas[np.array([4,20])]
i = -1
for g1 in gammas1:
    i += 1
    if i == len(tableau10):
    ...
    for g2 in gammas2:
        g = np.array([g1,g2])
        r = sel(first=True, t_delay=0, A0_pr=0, g=g,v=vs[1])
        wg, c1 = conductivity(r, order=1, plot=False, subtract=False, begin=t[0], end=t[-1])
        wg, c3 = conductivity(r, order=3, plot=False, subtract=False, begin=t[0], end=t[-1])
        c = (c1 + Ascale**2*c3*0)*u_conductivity
        plt.figure('cond-imp-real')
        if g1 >= gammas[14]:
            ...
            ls='-'
        plt.plot(wg*u_e*meV_to_THz,np.abs(c.real), label=f'$\gamma/2\Delta={str(np.round(g1,1))}$',ls=ls, c=tableau10[i])
        plt.xlabel('Frequency (THz)')
        plt.xlim((0,5.5))
        plt.ylim((0,1.7e5))
        plt.ylabel('$\sigma\,\'$ ($\Omega^{-1}$cm$^{-1}$)')
        ...
        plt.figure('cond-imp-imag')
        plt.plot(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$\gamma/2\Delta={str(np.round(g1,1))}$',ls=ls, c=tableau10[i])
        plt.xlabel('Frequency (THz)')
        plt.xlim((0,4*gap*u_e*meV_to_THz))
        # plt.ylim((0,100e4))
        plt.ticklabel_format(axis="y", style="sci", scilimits=(2,4))
        ...
        if g2 == gammas2[0]:
            plt.figure('cond-imp-legend')
            plt.plot(wg*u_e*meV_to_THz,np.abs(c.imag), label=f'$\gamma={str(np.round(g1*10,1))}$ meV',ls=ls, c=tableau10[i])
            plt.ylim((0,0.1))
            plt.legend()
...
plt.axvline(2*d_eq[1]*u_e*meV_to_THz, c='k', lw=0.3)
plt.tight_layout()
savefig('cond-imp-real2.pdf')

plt.figure('cond-imp-imag')
plt.axvline(2*gap*u_e*meV_to_THz, c='k', lw=0.3)
plt.tight_layout()
savefig('cond-imp-imag2.pdf')

plt.figure('cond-imp-legend')
savefig('cond-imp-legend2.pdf')
...
gammas1 = gammas
gammas2 = gammas[np.array([24])]
for g1 in gammas1:
    for g2 in gammas2:
        g = np.array([g1,g2])
        r = sel(first=True, t_delay=0, A0_pr=0, g=g,v=vs[1])

t,d, d_im, w_, dw_ = plotHiggs(r,plot=True)
plt.figure()
plt.plot(w_,dw_)
plt.xlim((0,5))
# %%
