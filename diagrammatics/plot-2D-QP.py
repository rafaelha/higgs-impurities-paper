...
v_list = [0.02,0.05,0.4]

w_list = np.linspace(0.00001,1,30)
T_list = np.linspace(0.00001,50,30)/u_temp
gammas =  [ np.array([10, 0.001]), np.array([0.001, 10]),  np.array([10,5]), np.array([0.001, 0.001])]
lab = ['dirty-clean', 'clean-dirty', 'dity-dirty', 'clean-clean']
# gammas =  [ np.array([10, 0.001]) ]
...
for v in v_list:
    ...
    for g in gammas:
        ...
        for r in res:
            if r['v']==v and (r['g']==gammas[gind-1]).all():
                def indexof(x, array):
                    return np.where(x==array)[0][0]
                JH2D[indexof(r['w'].real, w_list), indexof(r['T'],T_list)] = np.sum(r['jQP'])
                all_zeros = False
        ...
        plt.figure('QP-2D', figsize=(3,2.8))
        plt.clf()
        plt.ion()
        data = np.abs(np.nan_to_num(JH2D))

        vmax = np.max(data[:15,0:7])

        plt.pcolormesh(w_list*u_w,T_list*u_temp,data.T, cmap='cividis', vmax=vmax)
        ...
        # plt.xlim((0,1.4))
        plt.ylim((0,50))
        plt.title(lab[gind-1])
        plt.tight_layout()
        plt.savefig(f'qp-2d/v{vind}-g{gind}_abs.png', dpi=300)
        plt.savefig(f'qp-2d/v{vind}-g{gind}_abs.pdf', transparent=True)

        plt.figure('QP-2d-phase', figsize=(3,2.8))
        plt.clf()
        plt.ion()
        ...
        plt.ylim((0,50))
        plt.tight_layout()
        plt.savefig(f'qp-2d/v{vind}-g{gind}_phase.png', dpi=300)
        plt.savefig(f'qp-2d/v{vind}-g{gind}_phase.pdf', transparent=True)
        plt.pause(0.01)
