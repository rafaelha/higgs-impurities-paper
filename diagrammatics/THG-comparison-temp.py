...
folder = 'leggett-driving-comparisonT'
        ...
        for i, case in z(fourcases):
            ...
            for k, x in z(X):
                if temp:
                    ...
                    w = x
                r = sel(first=False,  w=w, T=T, v=v, g=case, QP=False)
                rqp = sel(first=False, w=w, T=T, v=v, g=case, QP=True)
                if r == []:
                    continue
                ...
                if True and k in [0,10,20,30,40,50,60, len(wss)-1]:
                    def spec(t1,j1, label=''):
                        ...
                        plt.xlabel('t')
                        plt.title(f'$w={np.round(w*u_w,3)}, T={np.round(T*u_temp,3)}$')
