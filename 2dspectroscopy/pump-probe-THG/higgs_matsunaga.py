...
for p in params:
    ...
    def A(t):
        ...
        return A0*np.exp(-(t-te)**2/(2*tau**2))*np.cos(w*t) \
            +  A0_pr*np.exp(-(t-te_pr-t_delay)**2/(2*tau_pr**2))*np.cos(w_pr*(t-t_delay))
    ...
    #         fw = fw[idx]
    #     return xw, fw
    # plt.plot(t_points,A(t_points))
    # tw, aw = rfft(t_points,A(t_points))
    # plt.figure()
    # plt.plot(tw,np.abs(aw))
    # plt.axvline(0.3*2)
    # plt.xlim((0,1))
    # plt.figure()
    # tw, aw = rfft(t_points,A(t_points)**2)
    # plt.plot(tw,np.abs(aw))
    # plt.axvline(0.3*2)
    # plt.xlim((0,1))
